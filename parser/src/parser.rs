...
fn take1(i: &[Token]) -> IResult<&[Token], Token> {
    let first_token = i
        .first()
        .ok_or_else(|| nom::Err::Error(NomError::from_error_kind(i, ErrorKind::Complete)))?;
    Ok((&i[1..], first_token.clone()))
}
    ...
    map(
        ...
        |(return_type, name, _, decl, _)| {
            (
                ...
                decl.iter()
                    .map(|(typ, name)| (typ.clone(), Variable { name: name.clone() }))
                    .collect(),
            )
        },
    )(i)
...
fn ret(i: &[Token]) -> IResult<&[Token], Insn> {
    map(
        pair(tok(Token::Keyword(Keyword::Return)), alt((ident, literal))),
        |(_, token)| {
            Insn::Return(match token {
                Token::Ident(id) => Value::Variable(Variable { name: id }),
                // TODO: no cast here.
